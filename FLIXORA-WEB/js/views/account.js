...
export async function AccountView(){
  ...
  wrap.innerHTML = `
    <section class="section account-page">
      ...
      <!-- profile modal will be created dynamically -->
    </section>
  `;
  ...
  function openProfileModal(){
    ...
    async function loadAvatars(catKey){
      avatarGrid.innerHTML = '<div class="comment-note" style="grid-column:1/-1;">Carregando imagens...</div>';
      try{
        ...
        const fallbackThumbs = (cat.fallbacks || []).map((url, index) => ({
          url,
          title: cat.name,
        ...
        const unique = [];
        const seen = new Set();
        for (const item of [...fallbackThumbs, ...allThumbs]) {
          if (!item.url || seen.has(item.url)) continue;
          seen.add(item.url);
          unique.push(item);
        }
        const thumbList = unique.slice(0, 56);
        ...
        thumbList.forEach(t => {
          ...
          btn.innerHTML = `<img src="${t.url}" alt="${t.name}" loading="lazy" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22100%22 height=%22100%22%3E%3Crect fill=%22%23212129%22 width=%22100%22 height=%22100%22/%3E%3Ccircle cx=%2250%22 cy=%2250%22 r=%2220%22 fill=%22%23A855F7%22/%3E%3C/svg%3E'"/>`;
          if ((selectedInput.value || photoURL) && selectedInput.value === t.url) {
    ...
    setTimeout(() => {
      const initialKey = AVATAR_CATEGORIES[0].key;
      selectedInput.value = photoURL || '';
      loadAvatars(initialKey);
    }, 100);
    ...
    mForm?.addEventListener('submit', async (ev)=>{
      ...
      try{
        ...
        const nextPhoto = selected || photoURL;
        ...
        window.dispatchEvent(new CustomEvent('profileUpdated', { detail: { displayName: nextName, photoURL: nextPhoto } }));
